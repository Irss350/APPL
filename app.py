...
def fetch_keyword_news(your_keyword_list, date, apiKey):
    ...
    for i in range(len(your_keyword_list)):
        ...
        for i in range(len(article)):
            if article[i]["content"] is None:
                continue
            else:
                all_news.append(cv_model.transform(nlp_preprocessing(article[i]["content"])))

        # filtered_list= list(filter(None, all_news))

        # for i in range(len(filtered_list)):
        #     in_list.append(cv_model.transform(nlp_preprocessing(filtered_list[i])))
...
if st.button("Fetch News and Predict"):
    ...
    if weekno<5:
        ...
        for i in range(len(input_list)):
            ...
            # inp= cv_model.transform(input_list[i])
            out_class_list.append(model.predict(input_list[i]))
